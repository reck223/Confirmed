export function RangeBar({ sl, entry, tp1, tp2, direction }: { sl: number; entry: number; tp1: number; tp2: number; direction: string }) {
  ...
  return (
    <div style={{ marginTop: 10 }}>
      <div style={{ position: 'relative', height: 4, borderRadius: 999, background: 'linear-gradient(90deg,#f87171,rgba(255,255,255,0.14),#4ade80)' }}>
        {markers.map(m => (
          <div key={m.label} style={{ position: 'absolute', left: `${pos(m.v)}%`, top: -3, transform: 'translateX(-50%)' }}>
            <div style={{ width: 10, height: 10, borderRadius: '50%', background: m.label === 'SL' || m.label === 'TP1' || m.label === 'TP2' ? 'transparent' : m.color, border: `2px solid ${m.color}` }} />
          </div>
        ))}
      </div>
      ...
      <p style={{ fontSize: 8, fontWeight: 700, color: '#fbbf24', marginTop: 6, letterSpacing: '0.02em' }}>
        ⚠ Planned levels only — no broker-side stop is set. Close manually in TradeLocker.
      </p>
    </div>
  )
}
